...
#' @param k Number of groups to evaluate (Example, 2:8)
#' @param reps Number of repeats, default=3
#' @param n Percent of data to mask, default= 0.05 (5 percent)
#' @param ... Arguments from RcppML:nmf function
#' @return object to be plotted using plot()
#' @import RcppML
#' @importFrom progress progress_bar
#' @export

joint_crossValidate <- function(data, k, reps = 3, n = 0.05, ...) {
  ...
  total_iterations <- reps * length(k)

  # Initialize progress bar using progress:: namespace
  pb <- progress::progress_bar$new(
    format = "  Processing [:bar] :percent | ETA: :eta | :current/:total iterations",
